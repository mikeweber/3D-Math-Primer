...
class EulerAngles
  ...
  def canonize
    @pitch = MathUtil.wrap_tau(@pitch)
    
    if @pitch < -MathUtil::quarter_tau
      @pitch     = -MathUtil::half_tau - @pitch
      @heading  += MathUtil::half_tau
      @bank     += MathUtil::half_tau
    ...
    if @pitch.abs > MathUtil::quarter_tau
      ...
      @bank = MathUtil.wrap_tau(@bank)
    end
    
    @heading = MathUtil.wrap_tau(@heading)
  end
end
